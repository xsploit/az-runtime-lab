...
| `force_wifi_off` | Escape hatch: turn the player's own Wi-Fi switch off at every start. Default false: the switch is left as the user set it, and the `wpa_cli` stand-in keeps its retry loop at ~1/s |
