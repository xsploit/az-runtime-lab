...
with (lab/'xvfb.log').open('w') as log:
 xv=subprocess.Popen(['/usr/bin/Xvfb','-displayfd',str(wr),'-screen','0','1280x800x24','-nolisten','tcp','-ac'],pass_fds=(wr,),stdout=log,stderr=log)
    ...
    if os.environ.get('NATIVE_NAVIGATION') and feedbackready:
     if navigationproc is None:
      ...
      if guest_pid is not None:
       ...
       navigationproc=subprocess.Popen([sys.executable,str(base/'analysis/run_native_navigation.py'),str(guest_pid),str(baseline),str(input_socket),str(navigation_socket),'--mapping',str(base.parent/'rx3-research/bitedj-mapping/Pioneer-DDJ-FLX6.midi.xml'),'--seconds','300'],stdout=navigationlog,stderr=navigationlog)
