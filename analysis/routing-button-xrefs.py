...
targets={0x3b864e8:'cross0',0x3b864f0:'cross2',0x3b864f8:'cross1',0x3b86630:'mono1',0x3b86638:'mono0',0x3b866e0:'link0',0x3b866e8:'link1',0x3b866f0:'link2',0x3b866f8:'link3'};out=[]
for s in ss:
 ...
 for i in range(0,s[5]-32,4):
  ...
  for j in range(1,9):
   ...
   if (x&0xff000000==0x91000000 or x&0xffc00000 in (0xf9000000,0xf9400000)) and (x>>5)&31==reg:
    dest=page+(((x>>10)&4095)*(8 if x&0xffc00000 in (0xf9000000,0xf9400000) else (4096 if x&(1<<22) else 1)))
...
(b/'analysis/az-routing-button-id-xrefs.json').write_text(json.dumps(out,indent=2));print(json.dumps(out,indent=2))
