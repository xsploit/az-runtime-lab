...
static double run(int opt,XImage*s,unsigned w,unsigned h,int count){double start=now();for(int i=0;i<count;i++){XImage d;int ok=opt?candidate(s,0,0,w,h,&d):baseline(s,0,0,w,h,&d);if(!ok)exit(2);checksum+=(unsigned char)d.data[(i*7)%(w*h*4)];if(opt)lab_scratch_free(d.data);else free(d.data);}return now()-start;}
