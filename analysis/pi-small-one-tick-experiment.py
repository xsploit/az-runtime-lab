...
def change(enable):
 ...
 lines+=['detach','quit'];fn=Path('/tmp/az-small-one-tick.gdb');fn.write_text('\n'.join(lines)+'\n')
 r=subprocess.run(['gdb','-q','-nx','-batch','-x',str(fn)],capture_output=True,text=True,timeout=10)
 Path(f'/tmp/az-small-one-tick-{"apply" if enable else "restore"}.log').write_text(r.stdout+r.stderr)
...
try:
  ...
  if a.timer_fields:
   with open(f'/proc/{a.pid}/mem','rb',buffering=0) as m:
    ...
    while time.monotonic()<end:
     ...
     time.sleep(.001)
   Path(f'/tmp/az-small-one-tick-fields-{label}.json').write_text(json.dumps(records))
  else:
   subprocess.run(['ffmpeg','-y','-loglevel','error','-f','x11grab','-framerate','120','-video_size','400x32','-i',':0+420,150','-t','6','-f','framemd5',f'/tmp/az-small-one-tick-{label}.framemd5'],check=True,timeout=12)
 ...
 change(False)
Path('/tmp/az-small-one-tick-results.json').write_text(json.dumps({'patches':[{'address':hex(a),'original':hex(o),'candidate':hex(n)} for a,o,n in patches],'samples':results,'restored':True},indent=2)+'\n')
