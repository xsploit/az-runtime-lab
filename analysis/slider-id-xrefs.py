...
ids=[0x0004d17e2dc19f00,0x7dcb7936b7ed2b00,0xa6518fb8028ca800,0xdacc2ca9faed3600]
hits=[]
for s in sections:
 ...
 for i in range(0,s[5]-64,4):
  ...
  for j in range(1,17):
   ...
   if x&0xff800000==0xf2800000 and x&31==reg:
    ...
    if value&~255 in ids:
     hits.append({'start':hex(s[3]+i),'end':hex(s[3]+i+j*4),'register':reg,'value':hex(value),'lane':ids.index(value&~255),'caveat':'Validate register writes and control flow'})
     break
out=base/'analysis/az-slider-id-xrefs.json';out.write_text(json.dumps(hits,indent=2)+'\n')
