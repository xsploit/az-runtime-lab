...
static FilterManager snap_manager(void){
 ...
 for(int j=0;j<7;j++){m.initialized[j]=inits[j];m.processed[j]=processes[j];}m.sweep_ticks=ticks;return m;
}
#endif
...
#endif
 int previous=c->manager.current;composed_process(c,actual,n);if(previous==1&&c->manager.current==6)to_crush++;if(previous==6&&c->manager.current==1)to_filter++;check(&c->filter);
...
int main(int argc,char **argv){
 ...
 for(unsigned j=0;j<18000;j++){
  if(j%19==0||j%41<4){const int types[]={0,1,6};int target=types[(j/3)%3];if(!filter_manager_request(&c.manager,target))busy++;
#ifndef HOST_REPLAY
...
#endif
 fclose(records);
 if(!to_crush||!to_filter)return 3;
