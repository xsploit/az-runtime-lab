...
#endif
static unsigned transitions[7][7];
...
int main(int argc,char **argv){
 ...
 for(unsigned j=0;j<18000;j++){
  if(j%19==0||j%41<4){const int types[]={0,1,2,6};int target=types[(j/3)%4];if(!filter_manager_request(&c.manager,target))busy++;
#ifndef HOST_REPLAY
...
#endif
 fclose(records);
 if(!to_crush||!to_filter||!transitions[1][2]||!transitions[2][1]||!transitions[6][2]||!transitions[2][6])return 3;
