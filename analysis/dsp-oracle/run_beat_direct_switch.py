...
(here/'beat-direct-native.S').write_text(asm)
...
files=['beat_direct_switch_probe.c','echo_quantize.c','echo_audio.c','echo_control.c']
gcc=root/'rx3-human-delegation/scratch/debbin/arm-linux-gnueabi-gcc'
cmd=[str(gcc),*flags,'-static','-mfpu=neon','-mfloat-abi=softfp',*files,'echo-control-native.S','beat-direct-native.S','-lm','-o','beat-direct-arm']
subprocess.run(cmd,cwd=here,check=True)
p=subprocess.run(['qemu-arm-static','./beat-direct-arm'],cwd=here,capture_output=True,text=True)
report={'firmware_sha256':sha,'native_ranges':ranges,'relocations':relocations,'command':cmd,'returncode':p.returncode,'stdout':p.stdout,'stderr':p.stderr,'scope':'Direct native switch callback with actual Echo/Delay controls. No audio execute, outer operate transitions or host reconstruction comparison.'}
report['source_hashes']={n:hashlib.sha256((here/n).read_bytes()).hexdigest() for n in files+['run_beat_direct_switch.py','beat_manager_probe.c','echo_quantize_compare.c','echo_audio_compare.c','echo_quantize.h','echo_audio.h','echo_control.h','beat-direct-native.S']}
(here/'beat-direct-switch-results.json').write_text(json.dumps(report,indent=2)+'\n')
