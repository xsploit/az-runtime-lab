...
files=['beat_manager_compare.c','beat_manager_echo.c','echo_quantize.c','echo_audio.c','echo_control.c']
...
report['source_hashes']={n:hashlib.sha256((here/n).read_bytes()).hexdigest() for n in files+['beat_manager_echo.h','beat_manager_probe.c','echo_quantize_compare.c','echo_quantize.h','echo_audio.h','echo_audio_compare.c','run_beat_manager_compare.py','beat-manager-native.S']}
(here/'beat-manager-results.json').write_text(json.dumps(report,indent=2)+'\n');print(json.dumps(results,indent=2));raise SystemExit(any(p['returncode'] for p in results.values()))
