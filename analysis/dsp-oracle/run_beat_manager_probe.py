...
files=['beat_manager_probe.c','echo_quantize.c','echo_audio.c','echo_control.c']
gcc=root/'rx3-human-delegation/scratch/debbin/arm-linux-gnueabi-gcc'
commands=[[str(gcc),*flags,'-static','-mfpu=neon','-mfloat-abi=softfp',*files,'echo-control-native.S','beat-manager-native.S','-lm','-o','beat-manager-probe-arm']]
for cmd in commands:subprocess.run(cmd,cwd=here,check=True)
p=subprocess.run(['qemu-arm-static','./beat-manager-probe-arm'],cwd=here,capture_output=True,text=True)
(here/'beat-manager-probe.stderr').write_text(p.stderr)
if p.returncode:print(p.stderr);raise SystemExit(p.returncode)
rows=json.loads(p.stdout);(here/'beat-manager-native-trace.json').write_text(json.dumps(rows,indent=2)+'\n')
print(json.dumps([r for i,r in enumerate(rows) if i==0 or any(r[k]!=rows[i-1][k] for k in ('type','phase','target','echo_on','active_object'))],indent=2))
