#ifndef HOST_REPLAY
#include "beat_delay_manager_probe.c"
#else
...
int main(int argc,char **argv){if(argc!=2)return 2;
...
#endif
 if(!records)return 2;
 DelayAudio s={0};s.control.rate=44100;s.control.max_samples=352800;s.control.depth=.8f;s.time_ms=20;s.delay=s.previous_delay=882;s.ring_length=352800;
 ...
 cp=(DelayPlayerFixture){0};cp.channel_valid=cp.beat_present=cp.grid_enabled=1;cp.count=16;for(unsigned i=0;i<=16;i++)cp.beats[i]=(DelayBeat){1,12000,i*500};cq=(DelayQuantize){0,0,1,0,0,0,-1,5};
#ifndef HOST_REPLAY
 Stereo scratch[64];
 set_native(&s,native_ring);set_player();set_quantize();obj[2]=44100;float inv=1.0f/44100;memcpy(&obj[3],&inv,4);obj[4]=64;obj[12]=50;obj[13]=100;obj[14]=(uint32_t)-100;obj[16]=2;obj[18]=9;obj[19]=0;obj[20]=0;
 vtable[2]=(uint32_t)delay_execute;vtable[4]=(uint32_t)delay_init;vtable[6]=(uint32_t)native_delay_beat;vtable[7]=(uint32_t)native_status;vtable[8]=(uint32_t)delay_off;vtable[9]=(uint32_t)delay_on;vtable[10]=(uint32_t)native_delay_notify;vtable[11]=(uint32_t)native_delay_keep;
 ...
 manager[5]=(uint32_t)obj;manager[22]=(uint32_t)scratch;
 DelayRamp f,d,l;ramp(&f,191,1);ramp(&d,191,0);ramp(&l,66112,0);memcpy(manager+24,&f,40);memcpy(manager+36,&d,40);memcpy(manager+46,&l,40);manager[56]=manager[57]=5;float depth=.8f;memcpy(manager+58,&depth,4);

#endif
 BeatManagerDelay m;beat_manager_delay_init(&m);
 ...
 if(initial){
  ...
  m.bfx_bpm=initial_bpms[initial_case/12];m.next_beat=initial_case%12;m.next_type=1;
 ...
 if(!initial){
...
#endif
   beat_manager_delay_set_beat(&m,&s,&cq,&cp,block%2);
  }
  Stereo audio[64],native_audio[64];for(unsigned i=0;i<64;i++)audio[i]=native_audio[i]=(Stereo){rnd()*.1f,rnd()*.1f};
#ifndef HOST_REPLAY
  native_manager_operate(manager,native_audio,64,bypass);BeatManagerDelay ref=capture_manager();DelayAudio ra=get_native();DelayQuantize rq=get_quantize();
#else
  BeatManagerDelay ref;DelayAudio ra;DelayQuantize rq;
#endif
  ...
  DelayQuantize local_q=cq;DelayPlayerFixture local_p=cp;memset(&cq,0,sizeof cq);memset(&cp,0,sizeof cp);
  beat_manager_delay_process(&m,&s,&local_q,&local_p,ring,audio,64,bypass);cq=local_q;cp=local_p;
#else
  beat_manager_delay_process(&m,&s,&cq,&cp,ring,audio,64,bypass);
#endif
  calls++;frames+=64;
  if(m.consumed){
#ifndef HOST_REPLAY
   native_manager_operate(manager,native_audio,64,bypass);
   BeatManagerDelay repeated=capture_manager();compare(&repeated,&ref,sizeof ref,"native-round-guard",0,block);
#endif
   beat_manager_delay_process(&m,&s,&cq,&cp,ring,audio,64,bypass);
  }
  compare(&m,&ref,sizeof m,"manager",0,block);compare(&s,&ra,sizeof s,"delay",0,block);compare(&cq,&rq,sizeof cq,"quantize",0,block);compare(audio,native_audio,sizeof audio,"audio",0,block);compare(ring,native_ring,s.ring_length*sizeof(Stereo),"ring",0,block);
 }
