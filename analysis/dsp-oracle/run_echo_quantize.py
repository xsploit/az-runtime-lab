...
ranges=[('native_quantize_set',0x9885c,0x989dc),('native_time_update',0x989dc,0x98b24),('native_quantize_check',0x98fa0,0x99030),('native_beat_position',0x99060,0x990a4),('native_adjust',0xb5b38,0xb5c80),('echo_execute',0xb6f68,0xb7530)]
...
(here/'echo-quantize-native.S').write_text(asm)
...
flags=['-std=c11','-O2','-ffp-contract=off','-fno-tree-vectorize','-Wall','-Wextra','-Werror','-DECHO_NATIVE_QUANTIZE']
...
commands=[[str(gcc),*flags,'-static','-mfpu=neon','-mfloat-abi=softfp',*files,'echo-control-native.S','echo-quantize-native.S','-lm','-o','echo-quantize-arm'],['cc',*flags,'-DHOST_REPLAY',*files,'-lm','-o','echo-quantize-host'],['cc',*flags,'-DHOST_REPLAY','-fsanitize=address,undefined','-fno-omit-frame-pointer',*files,'-lm','-o','echo-quantize-host-asan']]
for cmd in commands:subprocess.run(cmd,cwd=here,check=True)
results={}
for label,cmd in [('arm',['qemu-arm-static','./echo-quantize-arm']),('host',['./echo-quantize-host']),('sanitized',['./echo-quantize-host-asan'])]:
 p=subprocess.run([*cmd,'native-echo-quantize-records.bin'],cwd=here,capture_output=True,text=True)
 results[label]={'returncode':p.returncode,'stdout':p.stdout,'stderr':p.stderr}
report={'firmware_sha256':sha,'native_ranges':ranges,'relocations':relocations,'compiler_commands':commands,'results':results,'scope':'Real native Echo, quantize checks/time update/set-time/beat lookup/parameter dispatch. Player engine getters supplied by controlled fixtures; outer manager and quantized On scheduler not covered.'}
report['source_hashes']={n:hashlib.sha256((here/n).read_bytes()).hexdigest() for n in files+['echo_quantize.h','echo_audio.h','echo_audio_compare.c','run_echo_quantize.py','echo-quantize-native.S']}
(here/'echo-quantize-results.json').write_text(json.dumps(report,indent=2)+'\n');print(json.dumps(results,indent=2));raise SystemExit(any(p['returncode'] for p in results.values()))
