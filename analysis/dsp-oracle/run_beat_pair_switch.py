...
(here/'beat-pair-switch-native.S').write_text(asm)
...
files=['beat_pair_switch_compare.c','echo_quantize.c','echo_audio.c','echo_control.c','delay_quantize.c','delay_control.c','beat-port/beat_pair.c','beat-port/beat_echo.c','beat-port/beat_delay.c']
gcc=root/'rx3-human-delegation/scratch/debbin/arm-linux-gnueabi-gcc'
cmd=[str(gcc),*flags,'-static','-mfpu=neon','-mfloat-abi=softfp',*files,'echo-control-native.S','beat-pair-switch-native.S','-lm','-o','beat-pair-switch-arm']
subprocess.run(cmd,cwd=here,check=True)
p=subprocess.run(['qemu-arm-static','./beat-pair-switch-arm'],cwd=here,capture_output=True,text=True)
report={'firmware_sha256':sha,'native_ranges':ranges,'relocations':relocations,'command':cmd,'returncode':p.returncode,'stdout':p.stdout,'stderr':p.stderr,'scope':'Actual direct native switch callbacks versus combined candidate manager; complete modeled manager/Delay/Echo/quantization state. No audio-loop transition comparison.'}
report['source_hashes']={n:hashlib.sha256((here/n).read_bytes()).hexdigest() for n in files+['run_beat_pair_switch.py','beat_manager_probe.c','echo_quantize_compare.c','echo_audio_compare.c','echo_quantize.h','echo_audio.h','echo_control.h','beat-pair-switch-native.S','beat-port/beat_pair.h','beat-port/beat_echo.h','beat-port/beat_delay.h','delay_quantize.h','delay_control.h','delay_audio.h','beat_manager_echo.h']}
(here/'beat-pair-switch-switch-results.json').write_text(json.dumps(report,indent=2)+'\n')
