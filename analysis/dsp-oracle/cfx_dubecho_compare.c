...
int main(int argc,char **argv){
 ...
 for(unsigned j=0;j<6000;j++){
  if((j<3600&&(j%240==0||j%240==200||j%240==220))||(j>=3600&&(j%19==0||j%41<4))){const int types[]={4,1,4,2,4,3,4,6,4,0};int target=j%240==200?0:j%240==220?4:types[(j/240)%10];if(j>=3600){const int random_types[]={0,1,2,3,4,6};target=random_types[(j/3)%6];}if(!filter_manager_request(&c.manager,target))busy++;
#ifndef HOST_REPLAY
...
#endif
 fclose(records);fprintf(stderr,"Dub tails enter%u resume%u directFilter%u directSweep%u\n",transitions[4][7],transitions[7][4],transitions[4][1],transitions[4][3]);
 ...
 if(!busy||!transitions[4][2]||!transitions[2][4]||!transitions[4][6]||!transitions[6][4]||!transitions[4][7]||!transitions[7][4]||!transitions[4][1]||!transitions[4][3])return 3;
