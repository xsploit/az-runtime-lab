...
static void retarget(DelayRamp *r,float target){for(int k=0;k<2;k++){r->target[k]=target;r->step[k]=mul(sub(target,add(r->current[k],r->step[k])),r->inverse);}r->count=0;r->done=0;}
void delay_control(DelayControl *s,unsigned action){
 switch(action){
 case DELAY_INIT:s->counter=0;s->max_samples=s->rate<<3;break;
 case DELAY_SELECT:s->counter=0;delay_control(s,DELAY_ON);break;
 case DELAY_TIME:s->time_changed=1;break;
 case DELAY_ON:
  for(int k=0;k<2;k++){s->gate.current[k]=0;s->gate.target[k]=1;s->gate.step[k]=mul(1,s->gate.inverse);}s->gate.count=0;s->gate.done=0;
  for(int k=0;k<2;k++){s->dry.current[k]=s->dry.target[k]=1;s->dry.step[k]=0;s->wet.current[k]=s->wet.target[k]=s->wet.step[k]=0;}
  delay_control(s,DELAY_DEPTH);break;
 case DELAY_OFF:retarget(&s->gate,0);retarget(&s->dry,1);retarget(&s->wet,0);break;
 case DELAY_DEPTH:{
  float dry,wet;
