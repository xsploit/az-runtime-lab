...
int filter_manager_request(FilterManager *m,int type){
 if(type!=0&&type!=1&&type!=2&&type!=3&&type!=4&&type!=6)return -1;
...
void filter_manager_process(FilterManager *m,Stereo *in,Stereo *scratch,size_t n,const FilterManagerCallbacks *cb){
 ...
 if((m->phase==0||m->phase==5)&&m->current!=m->next){
  m->phase=1;
  int active=m->current==7?m->previous:m->current;
  if(m->next==0&&active==4&&cb->off(cb->context,active)){
   m->previous=m->current;m->current=m->next=7;m->phase=5;goto process_active;
  }
  if(m->current==7&&m->next==4&&m->previous==4){cb->on(cb->context,active);m->current=m->next;m->phase=0;goto process_active;}
  ...
  if(m->wet.done){
   if(old_phase==1||old_phase==2){
    ...
    if(m->current!=m->next){
     ...
     m->initialized[m->next]++;m->current=m->next;target(&m->wet,1);
    }
...
void lab_cfx_manager_init(LabCfxManager *c){
 memset(c,0,sizeof *c);dub_runtime_default(&c->dub);sweep_runtime_construct(&c->sweep);filter_manager_init(&c->manager,191);
...
void lab_cfx_manager_controls(LabCfxManager *c,float color,float parameter){
 int active=c->manager.current==7?c->manager.previous:c->manager.current;c->manager.color[active]=color;c->manager.parameter[active]=parameter;
...
static int effect_on(void*context,int type){LabCfxManager*c=context;return type==4?dub_on(&c->dub.state):0;}
static int effect_off(void*context,int type){LabCfxManager*c=context;return type==4?dub_off(&c->dub.state):0;}
