...
(here/'beat-delay-manager-native.S').write_text(asm)
...
flags=['-std=c11','-O2','-ffp-contract=off','-fno-tree-vectorize','-Wall','-Wextra','-Werror','-DDELAY_NATIVE_QUANTIZE','-DDELAY_STATUS_ORACLE','-DBEAT_MANAGER_COMPARE_BUILD']
files=['beat_delay_manager_compare.c','beat_manager_delay.c','delay_quantize.c','delay_audio.c','delay_control.c']
gcc=root/'rx3-human-delegation/scratch/debbin/arm-linux-gnueabi-gcc'
commands=[[str(gcc),*flags,'-static','-mfpu=neon','-mfloat-abi=softfp',*files,'delay-manager-control-native.S','beat-delay-manager-native.S','-lm','-o','beat-delay-manager-arm'],['cc',*flags,'-DHOST_REPLAY',*files,'-lm','-o','beat-delay-manager-host'],['cc',*flags,'-DHOST_REPLAY','-fsanitize=address,undefined','-fno-omit-frame-pointer',*files,'-lm','-o','beat-delay-manager-host-asan']]
for cmd in commands:subprocess.run(cmd,cwd=here,check=True)
results={}
for label,cmd in [('arm',['qemu-arm-static','./beat-delay-manager-arm']),('host',['./beat-delay-manager-host']),('sanitized',['./beat-delay-manager-host-asan'])]:
 p=subprocess.run([*cmd,'native-beat-delay-manager-records.bin'],cwd=here,capture_output=True,text=True)
 results[label]={'returncode':p.returncode,'stdout':p.stdout,'stderr':p.stderr}
report={'firmware_sha256':sha,'native_ranges':ranges,'relocations':relocations,'compiler_commands':commands,'results':results,'scope':'Actual outer BeatEffectManager operate/type/target callbacks composed with Off and Delay audio, status and quantize. 180 persistent 64-frame blocks; deselection/reselection without retained tails, target changes, bypass, depth, changing selected-player time/tempo and transport flags. Beat enums0..12, range adjustment at BPM3000/4000/12000/17405/99999/unavailable, force-time true/false notifications, manager quantize and force-change inputs, consumed-round guard. Other effect algorithms and real multi-channel insertion graph not covered.'}
report['source_hashes']={n:hashlib.sha256((here/n).read_bytes()).hexdigest() for n in files+['beat_manager_delay.h','beat_delay_manager_probe.c','delay_quantize_compare.c','delay_quantize.h','delay_audio.h','delay_audio_compare.c','run_beat_delay_manager.py','beat-delay-manager-native.S','delay-manager-control-native.S','delay_control.h','iir_reconstructed.h']}
(here/'beat-delay-manager-results.json').write_text(json.dumps(report,indent=2)+'\n');print(json.dumps(results,indent=2));raise SystemExit(any(p['returncode'] for p in results.values()))
