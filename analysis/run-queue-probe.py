...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix='az-queue-probe-') as temp:
        ...
        subprocess.run([args.cc, '-static', '-O2', str(Path(__file__).with_name('probe-queue.c')),
                        '-o', str(executable)], check=True, timeout=60)
        run = subprocess.run([args.qemu, str(executable), *map(str, paths)],
                             check=True, capture_output=True, text=True, timeout=15)
    report = json.loads(run.stdout)
    if report != dict(pushes=9000, pops=9000, growth_allocations=3000, rounds=3, mismatches=0):
    ...
    report.update(firmware_sha256=digest,
                  ...
                  limitation='Constructed single-thread queue; no concurrency, allocator failure, timer or full-player validation')
