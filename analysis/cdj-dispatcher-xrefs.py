...
import struct,json
b=Path(__file__).resolve().parents[1];p=b/'cdj3000x/rootfs/home/root/pdj/EP145';d=p.read_bytes();o=struct.unpack_from('<Q',d,40)[0];sz,n=struct.unpack_from('<HH',d,58);ss=[struct.unpack_from('<IIQQQQIIQQ',d,o+i*sz) for i in range(n)]
targets={0x3038420:'RxDataDispatcher primary vtable'};out=[]
for s in ss:
...
(b/'analysis/cdj3000x-dispatcher-xrefs.json').write_text(json.dumps(out,indent=2));print(json.dumps(out,indent=2))
