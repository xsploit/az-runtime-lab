...
ids=[100380229266576633, 44967780707707287, 101436443698816896, 44967780692599604, 44967780684508245, 13292284036551555, 25999066090316757, 21563813511354750, 21464718063769333, 114889534129015966, 16532838454078186, 5983001460374921, 130080931428055242, 119531094434351977, 28072064056671549, 28072970818216702, 28072069705821870, 75835247682839324, 21550876568734636, 28072208841468170]
hits=[]
for s in sections:
  ...
  for j in range(1,17):
   ...
   if x&0xff800000==0xf2800000 and x&31==reg:
    ...
    if value in ids:
     ...
     break
out=base/'analysis/az-browser-button-id-xrefs.json';out.write_text(json.dumps(hits,indent=2)+'\n')
