...
p=argparse.ArgumentParser();p.add_argument('pid',type=int);p.add_argument('--xserver',type=int,required=True);p.add_argument('--output',type=Path,required=True);p.add_argument('--seconds',type=int,default=12);p.add_argument('--launcher',type=int,required=True);p.add_argument('--clock-file',type=Path,required=True);a=p.parse_args()
...
assert b'run-az-memory-clock.py' in (launcher/'cmdline').read_bytes()
...
clockfile=a.clock_file
assert subprocess.check_output(['findmnt','-n','-o','FSTYPE','-T',str(clockfile)],text=True).strip()=='tmpfs'
def physical_on():
 ...
 clock_process=subprocess.Popen(['/tmp/az-drm-publish','/dev/dri/card0','36',str(clockfile),'180'],stdout=clock_log,stderr=clock_log,user=1000,group=1000)
...
try:
 pulse(4);change(60.018,'physical-trial');physical_on();pulse(2)
 print('PHYSICAL_CLOCK_TRIAL_READY_120_SECONDS',flush=True)
 started=time.monotonic()
 while time.monotonic()-started<120:
  assert clock_process.poll() is None,'Display helper exited'
  time.sleep(1)
finally:
 try:physical_off()
 finally:
  change(59.24,'final-restore');os.close(fd)
  print('PREVIOUS_CLOCK_RESTORED',flush=True)
