...
def measure(label,hz):
 ...
 while time.monotonic()-started<8:
  records.append([time.monotonic(),read(timer+0x78,'<d'),read(timer+0x80,'<d'),get_sequence()[1]/1e6]);time.sleep(.001)
 ...
 row={'phase':label,'hz':hz,'source_rates':rates,'cpu_percent_one_core':dict(zip(['player','xwayland','test_process'],[100*(y-x)/os.sysconf('SC_CLK_TCK')/elapsed for x,y in zip(c,cpus())]))}
...
get_sequence()
stop_event=threading.Event();clock_thread=None;clock_error=[];clock_stats={}
def publisher():
 previous=None
 try:
  while not stop_event.is_set():
   seq,stamp=get_sequence()
   if seq!=previous:
    if previous is not None:
     assert seq>previous
     clock_stats['skipped']+=seq-previous-1
    publish_timing('/home/pompu_5/az-native-lab/xdjaz/state/sys/module/rockchipdrm/parameters/vsync_time',stamp)
    clock_stats['published']+=1;previous=seq
   stop_event.wait(.0005)
 except Exception as e:clock_error.append(str(e))
def physical_on():
 global clock_thread
 ...
 stop_event.clear();clock_stats.update(published=0,skipped=0)
 clock_thread=threading.Thread(target=publisher,daemon=True);clock_thread.start();time.sleep(.1)
 assert clock_stats['published']>1 and not clock_error

def physical_off():
 global clock_thread
 stop_event.set()
 if clock_thread is not None:
  clock_thread.join(timeout=3);assert not clock_thread.is_alive(),'DRM writer did not stop'
  clock_thread=None
 assert (launcher/'stat').read_text().rsplit(')',1)[1].split()[19]==launcher_identity
