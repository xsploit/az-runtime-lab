...
def change(enable):
 ...
 lines+=['detach','quit'];fn=Path('/tmp/az-position-rate.gdb');fn.write_text('\n'.join(lines)+'\n')
 r=subprocess.run(['gdb','-q','-nx','-batch','-x',str(fn)],capture_output=True,text=True,timeout=10)
 Path(f'/tmp/az-position-rate-{"apply" if enable else "restore"}.log').write_text(r.stdout+r.stderr)
 ...
 for label,enabled in [('original',False),('candidate',True),('restored',False)]:
  ...
  if a.timer_fields:
   with open(f'/proc/{a.pid}/mem','rb',buffering=0) as m:
    ...
    owner=0x7fff0981c010
    m.seek(owner);assert int.from_bytes(m.read(8),'little')==0x2b6f208
    m.seek(owner+0x19808);posobj=int.from_bytes(m.read(8),'little')
    m.seek(owner+0x19848);modeobj=int.from_bytes(m.read(8),'little')
    m.seek(modeobj+0x1c8);assert int.from_bytes(m.read(4),'little') not in (1,2)
    end=time.monotonic()+6;last=None;records=[]
    while time.monotonic()<end:
     m.seek(timer+0x78);values=struct.unpack('<dd',m.read(16))
     m.seek(owner+0x19808);assert int.from_bytes(m.read(8),'little')==posobj
     m.seek(posobj+0xf8);position=int.from_bytes(m.read(8),'little',signed=True)
     values=(*values,position)
     if values!=last:records.append([time.monotonic(),*values]);last=values
     time.sleep(.001)
   Path(f'/tmp/az-position-rate-fields-{label}.json').write_text(json.dumps(records))
  else:
   subprocess.run(['ffmpeg','-y','-loglevel','error','-f','x11grab','-framerate','120','-video_size','850x170','-i',':0+180,90','-t','6','-f','framemd5',f'/tmp/az-position-rate-{label}.framemd5'],check=True,timeout=12)
  ...
  if not a.timer_fields:
   ...
   subprocess.run(['grim',f'/tmp/az-position-rate-{label}.png'],env=e,check=True,timeout=5)
finally:
 change(False)
Path('/tmp/az-position-rate-results.json').write_text(json.dumps({'patches':[{'address':hex(a),'original':hex(o),'candidate':hex(n)} for a,o,n in patches],'samples':results,'restored':True},indent=2)+'\n')
