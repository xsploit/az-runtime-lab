...
# main RR 12; joint: EP147 main AND Xwayland RR 12. Everything is reverted on
# exit; arm rollback-watchdog.sh separately before running.
set -u
KNOB=${1:?irq|audio|renderer|joint}; LAB=${AZ_LAB:-$HOME/az-native-lab}; B=$LAB/local/kiosk-swap-backup
...
restore() { for f in pflx-mode-menu pflx-az-session start-pflx-kiosk; do sudo -n install -m 0755 "$B/$f" "/usr/local/bin/$f"; done; echo 3 | sudo -n tee /proc/irq/111/smp_affinity >/dev/null; echo "restored"; }
