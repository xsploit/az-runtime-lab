...
if (d/'queues.txt').exists():
    for line in (d/'queues.txt').read_text().splitlines():
        m=re.match(r'^(\d+\.\d+) renderer_rq_sq=(\S+) (\S+) xwayland_rq_sq=(\S+) (\S+)',line)
        if m:rows.append((float(m.group(1)),)+m.groups()[1:])
...
if (d/'strace.txt').exists():
    for line in (d/'strace.txt').read_text().splitlines():
        m=re.match(r'^(\d+)\s+[\d:.]+\s+(p?poll)\((\[.*?\])(.*)$',line)
        if not m or m.group(1)!=main or 'fd=20' not in m.group(3):continue
        e=re.search(r'fd=20, events=([A-Z|]+)',m.group(3));pc[e.group(1) if e else '?']+=1
        r=re.search(r'fd=20, revents=([A-Z|]+)',m.group(4));rc[r.group(1) if r else 'none']+=1
print("--- renderer poll() on fd 20, whole trace: requested",pc.most_common(4),"revents",rc.most_common(4))
if xp:
    tr={};outs=[];ins=0
    for line in (d/'sched.txt').read_text().splitlines():
    ...
    for line in (d/'sched-stacks.txt').read_text().splitlines():
