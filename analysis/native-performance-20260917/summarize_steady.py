...
for a,b in zip(times,times[1:]):
    k=kinds(a,b);rows.append(dict(t=a-start,gap=(b-a)*1000,bucket='waveform' if k=={'waveform'} else 'other' if k else 'unsampled'))
wave=[r for r in rows if r['bucket']=='waveform']
def stats(rs):
...
out=dict(duration_s=round(rows[-1]['t'],1) if rows else 0,waveform=stats(wave),buckets=dict(Counter(r['bucket'] for r in rows)),per_minute=per_min,underruns=under,largest=[dict(t=round(r['t'],1),gap=round(r['gap'],1)) for r in sorted(wave,key=lambda r:-r['gap'])[:8]])
if '--json' in sys.argv:print(json.dumps(out,indent=1));sys.exit()
w=out['waveform'];print(f"  {out['duration_s']}s waveform intervals n={w['n']} median={w['median']} p99={w['p99']} max={w['max']} >25={w['over25']} >40={w['over40']} >100={w['over100']} | underruns={len(under)} at {under[:6]}")
