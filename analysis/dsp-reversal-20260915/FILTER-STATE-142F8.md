...
A private packet-phase provenance model currently derives both mode branches,22 stores per lane and all twelve state writebacks from reviewed inventories. Its nine mutation guards reject changed operands, predicates, memory bases, setup count and prologue/core/writeback instructions. That is an internal cross-check, not a public reproduction tool or independent arithmetic oracle; the full model still requires review before publication. Mixed-half intermediate reads are recorded and require explicit ancestry review. No live/audio validation follows from this state map.
