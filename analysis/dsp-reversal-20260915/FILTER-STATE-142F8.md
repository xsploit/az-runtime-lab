...
Timing audit notice: compact-header PROT load stalls were absent from the initial private timing parser. State-address mapping remains instruction-backed, but its phase/provenance validation claims must be rechecked with header metadata before use as proof of sample timing. See PACKET-SEMANTICS.md.
