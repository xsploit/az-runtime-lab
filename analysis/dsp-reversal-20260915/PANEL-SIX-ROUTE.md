...
## Player HUI registration: separate ordering still unresolved

Independent EP147 inspection at `0x6c673c`–`0x6c677c` establishes a six-entry registration loop. The four immediate pieces construct base ID `0xc4bddb099942f600`; the loop ORs indices 0 through 5 into that base before calling `0x6c5080`. Therefore the registered IDs are `0xc4bddb099942f600` through `0xc4bddb099942f605`. The diagnostic referenced at `0x6c67c8`–`0x6c67d4` identifies these as indexed Mixer Sound Color FX button observers. The loop bound and OR operation were checked directly in AArch64 disassembly; the diagnostic was checked at VA `0x25d4ea8`.

This proves six player-side IDs, not their correspondence with the MCU scan order above. Neither the ID suffix nor physical button placement establishes the effect name or panel bit. The unresolved bridge is the observer/status handling between these IDs and the MCU fields. Do not assign RX3 effect enum order to these AZ selectors without that bridge. This is static registration evidence, not an input-injection or live-control test.
