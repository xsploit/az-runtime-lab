...
`0x11819420` loads command halfword at absolute `0x11800aa6` (command index 83) at `0x1181942c` and compares it with persistent state word 160 (`0x11803280`) loaded at `0x11819424`; the branch at `0x11819434` selects buffer resets/rewrites, including loops writing global `0x1182cf50` and `0x1182cf70` around `0x11819444-0x11819532`. These buffers are outside the B14 state struct; their meaning is unresolved.

`0x1181a8a4` starts a six-iteration control loop. It loads command halfword 77 at `0x11800a9a` (`0x1181a8ac-0x1181a8c0`) and increments the command pointer by two bytes at `0x1181a968`, ending after six iterations at `0x1181a970`; thus indices 77-82 are read, subject to loop control. It compares each to a corresponding halfword in a global region starting at `0x1182d408` (`0x1181a8a8-0x1181a8f4`) and conditionally stores values/flags at `0x1181a934-0x1181a950`. This establishes a six-slot command reader but not named controls.
