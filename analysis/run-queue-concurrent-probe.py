#!/usr/bin/env python3
"""Execute AZ queue push/pop with a counted replacement allocator.
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix='az-queue-probe-') as temp:
        temp = Path(temp)
        paths = [temp/'queue.bin']
        for path, code in zip(paths, pages):
            path.write_bytes(code)
        executable = temp/'probe'
        subprocess.run([args.cc, '-static', '-O2', '-pthread', str(Path(__file__).with_name('probe-queue-concurrent.c')),
                        '-o', str(executable)], check=True, timeout=60)
        run = subprocess.run([args.qemu, str(executable), *map(str, paths)],
                             check=True, capture_output=True, text=True, timeout=15)
    report = json.loads(run.stdout)
    if {k:v for k,v in report.items() if k != 'growth_allocations'} != dict(producers=4, consumers=1, pushes=200000, pops=200000, mismatches=0):
        raise RuntimeError('Unexpected queue probe result')
    report.update(firmware_sha256=digest,
                  method='Original ARM64 queue push/pop under QEMU; operator-new call redirected to counted calloc',
                  limitation='Constructed four-producer single-consumer queue; no multiple consumers, allocator failure, timer or full-player validation')
