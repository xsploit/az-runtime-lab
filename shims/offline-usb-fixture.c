...
#include <errno.h>
#include <stdatomic.h>
static _Atomic(void*) output_pcm;
static atomic_uint output_rate;
...
int snd_pcm_open(void **p,const char *name,int stream,int mode){
 ...
 int r=f(p,name,stream,mode);if(r==0 && stream==0)atomic_store(&output_pcm,*p);fprintf(stderr,"LAB_AUDIO open name=%s stream=%d mode=%d result=%d\n",name,stream,mode,r);return r;
}
...
/* Diagnostic pacing: output only, after real accepted writes. No audible device. */
int snd_pcm_hw_params(void*p,void*params){
 int(*apply)(void*,void*)=alsa_symbol("snd_pcm_hw_params");
 int(*getrate)(const void*,unsigned*,int*)=alsa_symbol("snd_pcm_hw_params_get_rate");
 int r=apply(p,params);unsigned rate=0;int direction=0;
 if(r==0 && p==atomic_load(&output_pcm) && getrate(params,&rate,&direction)==0){
  atomic_store(&output_rate,rate);fprintf(stderr,"LAB_PACE output_rate=%u\n",rate);
 }
 return r;
}
static void pace(void*p,long frames){
 static __thread unsigned long long deadline;
 unsigned rate=atomic_load(&output_rate);
 if(p!=atomic_load(&output_pcm)||!rate||frames<=0)return;
 struct timespec now;clock_gettime(CLOCK_MONOTONIC,&now);
 unsigned long long ns=(unsigned long long)now.tv_sec*1000000000ULL+now.tv_nsec;
 if(!deadline || ns>deadline+100000000ULL)deadline=ns;
 deadline+=(unsigned long long)frames*1000000000ULL/rate;
 struct timespec until={deadline/1000000000ULL,deadline%1000000000ULL};
...
long name(void *p,buffer_type b,unsigned long frames){ \
 ...
 long r=f(p,b,frames); if(!strcmp(#name,"snd_pcm_writei") || !strcmp(#name,"snd_pcm_writen"))pace(p,r); \
...
void *dlsym(void *handle,const char *name){
 if(!strcmp(name,"snd_pcm_hw_params"))return (void*)&snd_pcm_hw_params;
...
/* Isolated USB1 fixture: one connection then one mount event per process. */
#include <fcntl.h>
#include <stdarg.h>
...
int open64(const char*path,int flags,...){
 ...
 mode_t mode=0;if((flags&O_CREAT)||((flags&O_TMPFILE)==O_TMPFILE)){va_list ap;va_start(ap,flags);mode=va_arg(ap,int);va_end(ap);}
