...
static void initr(EchoRamp *r,unsigned n,float v){memset(r,0,sizeof *r);r->length=n;r->inverse=1.0f/(float)n;r->done=1;snap(r,v);}
void beat_manager_echo_init(BeatManagerEcho *m){memset(m,0,sizeof *m);m->remembered=5;m->off_on=1;m->off_time=20;m->off_beat=5;m->off_max_time=64000;m->off_max_beat=11;m->bfx_bpm=12000;m->depth=.8f;m->beat=m->next_beat=5;initr(&m->fade,191,1);initr(&m->dry,191,0);initr(&m->long_ramp,66112,0);}
static void setbeat(BeatManagerEcho *m,EchoAudio *a,EchoQuantize *q,EchoPlayerFixture *p,unsigned effect,unsigned beat,unsigned write_time){
 if(beat>11)return;
 ...
 if(m->bfx_bpm!=UINT32_MAX){
  ...
  unsigned max=effect==5?4000:m->off_max_time;
  for(unsigned guard=0;guard<24;guard++){
   ...
   if(effect==5){q->beat_button=beat;if(write_time)a->time_ms=time;a->control.time_changed=1;}
   else{m->off_beat=beat;if(write_time)m->off_time=time;}
   break;
  }
 }
 if(effect==5&&echo_quantize_check(q,p)){
  echo_quantize_set(a,q,p);
...
void beat_manager_echo_set_beat(BeatManagerEcho *m,EchoAudio *a,EchoQuantize *q,EchoPlayerFixture *p,unsigned write_time){
 unsigned on=m->active==5?a->on:m->off_on;
 unsigned max=m->active==5?9:m->off_max_beat;
...
static void toggle(BeatManagerEcho *m,EchoAudio *a,EchoQuantize *q,EchoPlayerFixture *p,unsigned effect){if(effect==5)echo_effect_set_on(a,q,p,!a->on);else m->off_on=!m->off_on;}
static void notify(EchoAudio *a){a->control.counter=0;echo_control(&a->control,ECHO_ON);}
static void switchtype(BeatManagerEcho *m,EchoAudio *a,EchoQuantize *q,EchoPlayerFixture *p){
 if(m->type==m->next_type)return;
 unsigned old=m->active,selected=m->next_type;float depth=old==5?a->control.depth:m->off_depth;
 ...
 if(selected==5){m->off_max_time=4000;m->off_max_beat=9;setbeat(m,a,q,p,0,q->beat_button,m->quantize);}
 else setbeat(m,a,q,p,0,m->off_beat,m->quantize);
 m->beat=m->next_beat=m->off_beat;
 if(selected==5){m->off_time=a->time_ms;a->control.depth=depth;echo_control(&a->control,ECHO_DEPTH);if(!a->on)toggle(m,a,q,p,5);m->active=5;notify(a);}
 ...
 if(selected==5&&m->next_beat>9)m->next_beat=9;
 target(&m->fade,1);m->phase=2;
 if(old==5?a->on:m->off_on)toggle(m,a,q,p,old);
}
void beat_manager_echo_process(BeatManagerEcho *m,EchoAudio *a,EchoQuantize *q,EchoPlayerFixture *p,Stereo *ring,Stereo *in,unsigned n,unsigned bypass){
 if(m->consumed)return;
 if(m->previous_bypass!=bypass){if(!bypass&&m->type==5)notify(a);m->previous_bypass=bypass;}
 float depth=bypass?0:m->depth;
 if(m->active==5){if(bypass||fabsf(depth-a->control.depth)>0x1p-23f){a->control.depth=depth;echo_control(&a->control,ECHO_DEPTH);}}
 else if(bypass||fabsf(depth-m->off_depth)>0x1p-23f)m->off_depth=depth;
 if(m->beat!=m->next_beat){beat_manager_echo_set_beat(m,a,q,p,1);m->next_beat=m->beat;}
 if(m->phase!=1&&m->phase!=2){
  if(m->type!=m->next_type){
   ...
   else if(m->next_type==0&&m->active==5&&!bypass){toggle(m,a,q,p,5);target(&m->dry,1);start(&m->long_ramp);m->phase=3;}
   else{m->phase=1;target(&m->fade,0);target(&m->dry,0);snap(&m->long_ramp,1);}
  }else if(m->target!=m->next_target){if(m->active==5)echo_control(&a->control,ECHO_OFF);m->phase=1;target(&m->fade,0);target(&m->dry,0);snap(&m->long_ramp,1);}
  else if(m->phase==3){if(m->active==5)echo_control(&a->control,ECHO_KEEP);toggle(m,a,q,p,m->active);target(&m->dry,0);snap(&m->long_ramp,1);m->phase=0;}
 }
 Stereo scratch[64];
 if(m->active==5)beat_echo_process(a,q,p,ring,in,scratch,n);else memcpy(scratch,in,n*sizeof(Stereo));
 if(m->phase==1||m->phase==2){unsigned was=m->phase;for(unsigned i=0;i<n;i++){scratch[i].l=add(mul(scratch[i].l,m->fade.current[0]),mul(in[i].l,sub(1,m->fade.current[0])));scratch[i].r=add(mul(scratch[i].r,m->fade.current[1]),mul(in[i].r,sub(1,m->fade.current[1])));tick(&m->fade);}
  if(m->fade.done){if(was==2)m->phase=0;else{m->force_change=0;switchtype(m,a,q,p);if(m->target!=m->next_target){m->target=m->next_target;if(m->active==5)echo_control(&a->control,ECHO_ON);target(&m->fade,1);m->phase=2;}m->consumed=1;}}
 }
...
void beat_manager_begin_round(BeatManagerEcho *m){m->consumed=0;}
