...
with tempfile.TemporaryDirectory(prefix='az-dsp-stream-') as tmp:
 ...
 try:
  ...
  assert [x['result'] for x in fx]==[1,0,1,0,1,1],fx
  ...
  print(json.dumps({'frames':len(out),'cue_max_error':cue_error,'filtered_master_difference_rms':delta,'crush_master_difference_rms':crush_delta,'noise_master_difference_rms':noise_delta,'filter_results':fx,'status':'pass'}))
 finally:
