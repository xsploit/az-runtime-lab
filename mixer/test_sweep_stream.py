 ...
 try:
  ...
  with (p/'in').open('wb',buffering=0) as w:
   for block in range(360):
    ...
    if block==100:sock.send(b'F1 0 2 .85 .6')
    if block==150:sock.send(b'F1 0 3 1 .5')
    if block==220:sock.send(b'F1 0 1 0 .5')
    if block==270:sock.send(b'F1 0 3 .05 .5')
    data=[]
    for i in range(64):
  ...
  tail=list(zip(out[180*64:210*64],samples[180*64:210*64]));delta=math.sqrt(sum((row[0]-.25*(src[0]+src[2]))**2 for row,src in tail)/len(tail));assert delta>.005,delta
  events=[json.loads(line) for line in err.decode().splitlines()];fx=[x for x in events if 'filter_frame' in x]
  assert [x['result'] for x in fx]==[1,0,1,0,1,1,1,1],fx
  crush=list(zip(out[70*64:95*64],samples[70*64:95*64]));crush_delta=math.sqrt(sum((row[0]-.25*(src[0]+src[2]))**2 for row,src in crush)/len(crush));assert crush_delta>.005,crush_delta
  noise=list(zip(out[120*64:145*64],samples[120*64:145*64]));noise_delta=math.sqrt(sum((row[0]-.25*(src[0]+src[2]))**2 for row,src in noise)/len(noise));assert noise_delta>.005,noise_delta
  print(json.dumps({'frames':len(out),'cue_max_error':cue_error,'sweep_master_difference_rms':delta,'crush_master_difference_rms':crush_delta,'noise_master_difference_rms':noise_delta,'filter_results':fx,'status':'pass'}))
 finally:
